...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const cmpRes = await fetch('/api/compare?name=' + encodeURIComponent(p.product_name) + '&current_price=' + encodeURIComponent(p.price || 0) + '&current_website=' + encodeURIComponent(p.website || '') + '&current_link=' + encodeURIComponent(p.product_link || '') + '&current_data_source=' + encodeURIComponent(p.data_source || 'historical'), { cache: 'no-store' });
...
function renderDetail(container, p, sumData, cmpData) {
  ...
  const score          = p.value_score ?? p.ai_score ?? 0;
  const specs          = parseSpecs(p.product_description);
  const summary        = (sumData && sumData.summary) || '';
  const listings       = ((cmpData && cmpData.listings) || []).filter(function(l){ return !(p.product_link && l.product_link === p.product_link); });
  ...
  if (listings.length > 0) {
    ...
    listings.sort(function(a,b){return a.price-b.price;}).forEach(function(l) {
      ...
      listingsHtml += '<div class="listing-row' + (l.badge ? ' listing-highlighted' : '') + '">' +
        ...
        '<div class="lr-score">Value: ' + (l.value_score != null ? l.value_score : l.ai_score != null ? l.ai_score : 'N/A') + '</div>' +
        '<a href="' + (l.product_link||'#') + '" target="_blank" rel="noopener noreferrer" class="lr-btn">View Listing →</a>' +
        '</div>';
    });
  ...
  const predictSection =
    '<div style="margin-top:40px;background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:28px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">' +
      ...
      '</div>' +
      '<button class="btn-secondary" onclick="openPredictDetail(\'' + escAttr(p.product_name) + '\')">Run Price Forecast</button>' +
    '</div>';
    ...
    '<div class="detail-grid">' +
      '<div class="detail-info">' +
        ...
        '<div class="detail-ai-score">' +
          '<div class="score-row"><span class="score-label">⚡ Value Score</span><span class="score-val">' + score + ' / 100</span></div>' +
          '<div class="detail-score-bar"><div class="detail-score-fill" style="width:' + score + '%"></div></div>' +
          '<p style="font-size:12px;color:var(--text3);margin-top:8px">Historical Value Score: price competitiveness, rating and review signal.</p>' +
        '</div>' +
        (summary ? '<div class="detail-summary-box"><h4>🤖 Smart Summary</h4><p>' + renderMarkdown(summary) + '</p></div>' : '') +
        bestDealHtml +
        '<div class="detail-actions">' +
          '<a href="' + (p.product_link||'#') + '" target="_blank" rel="noopener noreferrer" class="btn-primary">View ' + escHtml(p.website||'Store') + ' Listing →</a>' +
        '<button class="btn-secondary" onclick="openCompareDetail(\'' + escAttr(p.product_name) + '\', ' + Number(p.price || 0) + ', \'' + escAttr(p.website || '') + '\', \'' + escAttr(p.product_link || '') + '\')">Compare Prices</button>' +
        '</div>' +
      '</div>' +
    '</div>' +
...
// ===== MODALS =====
async function openCompareDetail(name, currentPrice, currentWebsite, currentLink) {
  const modal = showModal('Price Comparison');
  try {
    const res = await fetch('/api/compare?name=' + encodeURIComponent(name) + '&current_price=' + encodeURIComponent(currentPrice || 0) + '&current_website=' + encodeURIComponent(currentWebsite || '') + '&current_link=' + encodeURIComponent(currentLink || '') + '&current_data_source=historical', { cache: 'no-store' });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || data.live_error || ('Comparison request failed (' + res.status + ')'));
    modal.querySelector('.modal-body').innerHTML = renderCompareHtml(data);
  } catch (err) {
    modal.querySelector('.modal-body').innerHTML = '<p style="color:var(--red)">' + escHtml(err && err.message ? err.message : 'Failed to load comparison.') + '</p>';
  }
}

async function openPredictDetail(name) {
  const productPrice = (window.__detailProduct && window.__detailProduct.price) || 0;
  if (forecastChart) { forecastChart.destroy(); forecastChart = null; }
  const modal = showModal('Price Forecast');
  try {
    const res  = await fetch('/api/predict?name=' + encodeURIComponent(name) + '&days=30&current_price=' + encodeURIComponent(productPrice));
...
function renderCompareHtml(data) {
  if (data.error) return '<p style="color:var(--text2)">' + escHtml(data.error) + '</p>';
  const listings = data.listings || [];
  const best     = data.best_deal;
  const bestVal  = data.best_value;
  const savings  = (data.price_range && data.price_range.savings) || 0;
  if (!listings.length) return '<p style="color:var(--text2);padding:24px 0">No relevant live listings were returned for this product.</p>';
...
function getSiteClass(website) {
  ...
  if (w.includes('amazon')) return 'site-amazon'; if (w.includes('flipkart')) return 'site-flipkart'; if (w.includes('bestbuy')) return 'site-bestbuy'; if (w.includes('ebay')) return 'site-ebay';
  return '';
}
